...
TEST(SimpleStorage, ReadWritePartialAddress){
    std::filesystem::remove(std::filesystem::path{filename});
      SimpleFileStorage<FILESIZE> storage{FileRMA<FILESIZE>{filename}};

    auto address = storage.get_random_address(256);
    auto addr1 = address.subrange(0, 4);
    auto addr2 = address.subrange(4, 4);
    auto addr3 = address.subrange(8, 4);
    auto addr4 = storage.get_random_address(256);
    {
        auto writeb = storage.writeb(addr1);
        writeb.get<int>()[0] = 5;
        EXPECT_EQ(Result::Success, storage.commit(writeb));
    }
    {
        auto writeb = storage.writeb(addr2);
        writeb.get<int>()[0] = 8;
        EXPECT_EQ(Result::Success, storage.commit(writeb));
    }
    {
        auto writeb = storage.writeb(addr3);
        writeb.get<int>()[0] = 13;
        EXPECT_EQ(Result::Success, storage.commit(writeb));
    }
    {
        auto writeb = storage.writeb(addr4);
        writeb.get<int>()[0] = 20;
        EXPECT_EQ(Result::Success, storage.commit(writeb));
    }
    EXPECT_EQ(storage.readb(addr1).get<int>()[0], 5);
    EXPECT_EQ(storage.readb(addr2).get<int>()[0], 8);
    EXPECT_EQ(storage.readb(addr3).get<int>()[0], 13);
    EXPECT_EQ(storage.readb(addr4).get<int>()[0], 20);
}
